...
* Fix `chart::` macro charts being unreadable in dark mode: Chartist's default stylesheet hardcodes grid lines and axis labels to a fixed black-based color, so they were nearly invisible against the dark-mode background; both now switch to a light color under `prefers-color-scheme: dark`

== 4.1.1 (2026-08-12)

* Fix a crash (`TypeError: can't access property "url", sender.tab is undefined`) breaking rendering and every `include::` resolution on Firefox when a `file://` document is opened in a tab that isn't yet fully registered with the extension messaging API, e.g. set as the browser's start page (#734)

== 4.1.0 (2026-08-07)

* Add a `Release` GitHub Actions workflow (`workflow_dispatch`, pass a version) that bumps the version everywhere it's recorded (`package.json`, `app/manifest.json`, the Safari Xcode project), rolls this changelog's "Unreleased" section into a dated one, builds and tests the extension, signs the Firefox package with addons.mozilla.org so it can be installed manually outside the store, and publishes a GitHub release with the Chrome/Opera/Firefox zips and the signed Firefox `.xpi` as assets
