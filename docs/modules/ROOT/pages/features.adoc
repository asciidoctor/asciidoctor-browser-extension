...
Save the rendered document as a standalone HTML file, matching what running `asciidoctor file.adoc -o file.html` on the command line would produce.

Trigger the export either from:

* the right-click menu on a rendered page, by selecting *Export as HTML*
* the `Alt+Shift+E` keyboard shortcut

NOTE: This feature relies on the `downloads` API, which isn't available in every browser (e.g. Safari, Firefox for Android), so it isn't offered there.

== Custom favicon

Set the `:favicon:` document attribute to change the browser tab's icon while previewing the document:
...
The href is resolved against the current page URL, the same way a relative image reference in the document would be.
Remove the attribute (or reload without it) to restore the tab's original icon.
