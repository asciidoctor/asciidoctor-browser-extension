...
|Preview files with `ad` extension
|By default, the extension will only render `adoc` and `asciidoc` files.
Enable this to also preview files with the `ad` extension.
|Unchecked

|Preview files with `asc` extension
|Enable this to also preview files with the `asc` extension.
|Unchecked
