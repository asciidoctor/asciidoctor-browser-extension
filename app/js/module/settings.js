...
asciidoctor.browser.settings = (webExtension, Constants) => {
   ...
   */
  module.isTxtExtAllowed = async () =>
    (await module.getSetting(Constants.ALLOW_TXT_EXTENSION_KEY)) === 'true'

  /**
   * Is the Kroki extension enabled ?
   * @returns {Promise<boolean>}
   */
  module.isKrokiEnabled = async () =>
    (await module.getSetting(Constants.ENABLE_KROKI_KEY)) === 'true'

  /**
   * Get the Kroki server URL.
   * @returns {Promise<String>}
   */
  module.getKrokiServerUrl = async () =>
    module.getSetting(Constants.KROKI_SERVER_URL_KEY)

  /**
   * Is the extension currently enabled ?
   * @returns {Promise<boolean>}
   */
  module.isExtensionEnabled = () =>
    module.getSetting(Constants.ENABLE_RENDER_KEY)

  /**
   * Is "LiveReload" currently enabled on the page ?
   * @returns {Promise<boolean>}
   */
  module.isLiveReloadDetected = () =>
    module.getSetting(Constants.LIVERELOADJS_DETECTED_KEY)

  /**
   * Get the local poll frequency defined in the options page.
   */
  module.getLocalPollFrequency = async () => {
    const pollFrequency = await module.getSetting(
      Constants.LOCAL_POLL_FREQUENCY_KEY,
    )
  ...
  module.getRemotePollFrequency = async () => {
    const pollFrequency = await module.getSetting(
      Constants.REMOTE_POLL_FREQUENCY_KEY,
    )
    if (typeof pollFrequency === 'undefined') {
      // If the poll frequency is not defined, by default use 2 seconds
      return 2
  ...
  const getCustomScriptContent = async (customJavaScriptName) =>
    customJavaScriptName
      ? module.getSetting(Constants.CUSTOM_JS_PREFIX + customJavaScriptName)
      : undefined
